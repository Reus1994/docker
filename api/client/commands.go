...
func (cli *DockerCli) CmdCgroup(args ...string) error {
	...
	body, _, err := readBody(cli.call("POST", "/containers/"+name+"/cgroup?"+v.Encode(), cgroupData, nil))
...
func (cli *DockerCli) CmdMetric(args ...string) error {
	...
	name := cmd.Arg(0)
	body, _, err := readBody(cli.call("GET", "/containers/"+name+"/metric", nil, nil))
	if err != nil {
		return err
	}
...
func (cli *DockerCli) CmdSweep(args ...string) error {
	...
	var encounteredError error
	for _, name := range cmd.Args() {
		_, _, err := readBody(cli.call("POST", "/containers/"+name+"/sweep", nil, nil))
		if err != nil {
			fmt.Fprintf(cli.err, "%s\n", err)
...
func (cli *DockerCli) CmdLimit(args ...string) error {
	...
	var encounteredError error
	for _, name := range cmd.Args() {
		_, _, err := readBody(cli.call("POST", "/containers/"+name+"/limit?"+v.Encode(), data, nil))
		if err != nil {
			fmt.Fprintf(cli.err, "%s\n", err)
